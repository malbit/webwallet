...
public:
	...
	s_shouldExit = true;
	OMINFO << "Request to finish the arqwallet received";
	cv.notify_one();
    }
...
auto arqma_log_level =
         *(opts.get_option<size_t>("arqma-log-level"));

if (arqma_log_level < 1 || arqma_log_level > 4)
{
    cerr << "arqma-log-level,m option must be between 1 and 4!!!\n";
    return EXIT_SUCCESS;
}

// setup arqma logger
mlog_configure(mlog_get_default_log_path(""), true);
mlog_set_log(std::to_string(arqma_log_level).c_str());
...
auto current_bc_status
        = make_shared<xmreg::CurrentBlockchainStatus>(
            ...
            std::make_unique<xmreg::RPCCalls>(bc_setup.daemon_url));
...
if (!current_bc_status->init_arqma_blockchain())
{
...
xmreg::ArqmaRequests open_arqma(mysql_accounts, current_bc_status);
...
// restbed will be running and handling requests
...
// we are going to wait here for as long as "CTRL+C" hasn't been pressed
{
...
OMINFO << "Stoping restbed service.";
service.stop();
restbed_service.join();

OMINFO << "Stoping blockchain_monitoring_thread. Please wait.";
current_bc_status->stop();
blockchain_monitoring_thread.join();

OMINFO << "Stoping mysql_ping. Please wait.";
mysql_ping.stop();
