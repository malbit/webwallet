...
MysqlPing::operator()()
{
    ...
    // the while loop below is going to execute faster than ping_time specified.
    // The reason is so that we can exit it in a timely manner
    // when keep_looping becomes false.
    uint64_t between_ping_counter {0};

    // if ping_time lower than thread_sleep_time, use ping_time for thread_sleep_time then.
    thread_sleep_time = std::min(thread_sleep_time, ping_time);

    // we are going to ping mysql every no_of_loops_between_pings iterations of the while loop.
    uint64_t no_of_loops_between_pings = std::max<uint64_t>(1,
    ...
    OMINFO << "Exiting Mysql/mariaDB ping thread loop.";
}
