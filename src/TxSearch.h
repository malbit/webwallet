...
private:
    ...
    // this actually indicates whether thread loop finished its execution
    std::atomic_bool searching_is_ongoing {false};

    // marked true when we set new searched block value// from other thread. for example, when we import account
    // we set it to 0
