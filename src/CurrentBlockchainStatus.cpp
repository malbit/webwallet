...
CurrentBlockchainStatus::monitor_blockchain()
{
    ...
    if (!is_running)
    {
       ...
       while (true)
       {
           ...
           OMINFO << "PoolQueue size: " << TP::DefaultThreadPool::queueSize();
...
CurrentBlockchainStatus::update_current_blockchain_height()
{
    ...
    // but it also serves as a "ping" into the Arqma
    ...
    auto future_result = thread_pool->submit(
            ...
            -> bool
        {
            try
            {
                ...
                if (this->tx_exists(tx_hash, tx_index))
                {
...
{
    uint64_t fee_per_kB = get_dynamic_per_kb_fee_estimate();
...
//         << current_blockchain_height << '\n';
    ...
    {
      OMERROR << "Can not convert Payment_ID to pod: " << payment_id_str;
      return false;
    }
